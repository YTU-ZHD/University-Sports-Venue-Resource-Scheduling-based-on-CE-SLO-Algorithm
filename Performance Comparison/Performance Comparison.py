...
def load_requests_from_csv(path: str) -> List[Request]:
    with open(path, newline='', encoding='utf-8') as f:
        ...
        if {"requestid", "type", "desiredday", "desiredslot", "duration"} <= set(idx.keys()):
            ...
            return reqs
        needed = ["id", "type", "preferred_day", "preferred_start", "preferred_court", "duration"]
        for key in needed:
    ...
    def eval(self, indiv: Individual) -> Individual:
        ...
        for i, gene in enumerate(indiv.genes):
            ...
            if gene == 0:
                ...
                continue
            d, c, s = dec(gene)
            for t in range(s, s + r.duration): used[d][c][t] = 1
...
def initial_individual(reqs: List[Request]) -> Individual:
    ...
    for i in idxs:
        ...
        if r.rtype == HARD:
            ...
            if not placed:
                for c in courts:
                    if try_embed_place_with_eviction(occ, reqs, genes, r, d, c, s): placed = True; break
        else:
            for d, c, s in cands:
                if occ.can_place(r, d, c, s):
                    occ.place(r, d, c, s);
            ...
            if (not placed) and (r.rtype in MUST_HAVE):
                for d, c, s in cands:
                    if d == r.pref_day and try_embed_place_with_eviction(occ, reqs, genes, r, d, c,
                                                                         s): placed = True; break
...
def crossover_constructive(reqs: List[Request], p1: Individual, p2: Individual) -> Individual:
    ...
    for i in idxs:
        ...
        if r.rtype == HARD:
            ...
            if not placed:
                for c0 in courts:
                    if try_embed_place_with_eviction(occ, reqs, child, r, d0, c0, s0): placed = True; break
        else:
            if not placed:
                if chosen:
                    d, c, s = dec(chosen)
            ...
            if (not placed) and (r.rtype in MUST_HAVE):
                for d, c, s in cands:
                    if d == r.pref_day and try_embed_place_with_eviction(occ, reqs, child, r, d, c,
                                                                         s): placed = True; break
...
class BeliefSpace:
    ...
    def accept(self, pop: List[Individual]):
        ...
        for x in cand + self.pool:
            keyg = tuple(x.genes)
            if keyg not in seen: seen.add(keyg); unique.append(x)
...
class GAParams:
    ...
    burn_in_frac: float = 0.20
    p_active0: float = 0.85
    p_passive0: float = 0.30
    p_active_min: float = 0.40
    p_passive_min: float = 0.10
    anneal_alpha: float = 0.5
...
def evolve_single_region(reqs: List[Request], fit: Fitness, gargs: GAParams, seed: int = 0, mode: str = "GA") -> Tuple[
    ...
    G = effective_generations(gargs.pop_size, 1,
    ...
    for gen in range(1, G + 1):
        ...
        while len(new_pop) < gargs.pop_size:
            p1 = roulette(pop)
            child = crossover_constructive(reqs, p1, roulette(pop)) if random.random() < gargs.pc else p1.clone()
            child = mutate(reqs, child, gargs.pm)

            if mode == "SLO":
                if random.random() < p_act: child = active_learning_toward_teacher(reqs, child, elites[0])
                if random.random() < p_pas: child = passive_learning(reqs, child, roulette(pop))
        ...
        if mode in ("CGA", "SLO") and (gargs.influence_period and gen % gargs.influence_period == 0):
            ...
            for ind in pop: fit.eval(ind)

        region_best = max(pop, key=lambda z: z.fitness)
        if region_best.fitness > best.fitness: best = region_best
...
def evolve_ce_slo_parallel(reqs: List[Request], fit: Fitness, gargs: GAParams, seed: int = 0) -> Tuple[
    Individual, List[float]]:
    random.seed(seed)
    regions: List[Region] = []
    ...
    with ProcessPoolExecutor(max_workers=workers) as ex:
        for gen in range(1, G + 1):
            ...
            for ri, reg in enumerate(regions):
                last_imm_gen[ri] = immigrate_if_needed(reg.pop, fit, reqs, gargs.diversity_min, gargs.immigrants_rho,
                                                       gargs.immigrants_cooldown, gargs.immigrants_elite_keep, gen,
                                                       last_imm_gen[ri])
...
if __name__ == "__main__":
    ...
    args_single = GAParams(parallel=False, regions=1, use_fe_budget=False, gens=args.gens)
    args_ce = GAParams(parallel=True, regions=10, use_fe_budget=False, gens=args.gens)

    final_stats = {}
    mean_curves = {algo: [0.0] * (args.gens + 1) for algo in ALGORITHMS}

    for algo in ALGORITHMS:
        print(f"\n=== Running {algo} ({NUM_RUNS} runs) ===")
        run_fitnesses = []
        run_histories = []

        for run in range(NUM_RUNS):
            current_seed = args.seed + run
            print(f"Run {run + 1}/{NUM_RUNS} for {algo}...")

            if algo == "CE_SLO":
                best, history = evolve_ce_slo_parallel(reqs, fit, args_ce, seed=current_seed)
            else:
                best, history = evolve_single_region(reqs, fit, args_single, seed=current_seed, mode=algo)

            run_fitnesses.append(best.fitness)
            run_histories.append(history)

        mean_fit, std_fit, ci_fit = calc_statistics(run_fitnesses)
        final_stats[algo] = (mean_fit, std_fit, ci_fit)

        for gen in range(args.gens + 1):
            mean_curves[algo][gen] = sum(rh[gen] for rh in run_histories) / NUM_RUNS

    print("\n" + "=" * 50)
    print(f"Statistical Summary over {NUM_RUNS} Independent Runs")
    print("=" * 50)
    print(f"{'Algorithm':<10} | {'Mean':<8} | {'Std Dev':<8} | {'95% CI':<8}")
    print("-" * 50)
    ...
    with open("comparison_statistics.csv", "w", newline='') as f:
        ...
        for algo in ALGORITHMS:
            mean_fit, std_fit, ci_fit = final_stats[algo]
            print(f"{algo:<10} | {mean_fit:.5f} | {std_fit:.5f} | ±{ci_fit:.5f}")
            writer.writerow([algo, mean_fit, std_fit, ci_fit])

    with open("comparison_mean_curves.csv", "w", newline='') as f:
        writer = csv.writer(f)
        header = ["Iteration"] + ALGORITHMS
        writer.writerow(header)
        for gen in range(args.gens + 1):
            row = [gen] + [mean_curves[algo][gen] for algo in ALGORITHMS]
            writer.writerow(row)

    print("\n[INFO] 'comparison_statistics.csv' and 'comparison_mean_curves.csv' generated.")